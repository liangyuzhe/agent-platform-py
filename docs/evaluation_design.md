...
| `route_accuracy` | 复杂查询路由模式是否命中标注结果 | 0-1，最大 1.0 | 越大越好 | 衡量任务类型、关系图连通性和 JOIN 风险是否被正确路由到单 SQL、计划模式或澄清 |
...
复杂查询路由评测与 Schema Recall@K 不同：Recall@K 衡量“有没有把正确表召回到 TopK”，而 `route_accuracy` 衡量系统在选表和补表后是否选择正确执行模式。例如分析型问题应进入 `complex_plan`，明细导出或敏感数据问题应该进入 `clarify`，关系图断开的默认请求也应澄清范围。这类评测不要求调用外部 LLM，用 `task_type` 标注模拟数据库规则命中的任务类型，再由本地 `assess_feasibility` 结合关系图连通性和 JOIN 风险计算 `execution_mode`。
...
```json
{"query": "收入成本预算回款费用之间的关系", "tables": ["t_1", "t_2", "..."], "task_type": "analysis", "expected_route": "complex_plan"}
{"query": "员工工资和部门角色权限", "tables": ["t_1", "t_2", "..."], "task_type": "detail", "expected_route": "clarify"}
```
...
| `task_type` | 数据库规则命中的任务类型，不在代码里写死业务关键词 |
